...
# Define the ONNX model file path (using Railway persistent storage)
onnx_model_path = "/data/deepfake_model.onnx"

# GitHub Release URL for your ONNX model (327 MB)
...
def download_model():
    if not os.path.exists(onnx_model_path):
        ...
        if response.status_code == 200:
            ...
            # Check if file size is reasonable (e.g., at least 300 MB)
...
# Optional: Log first 100 bytes of the file to ensure it's binary and not HTML
...
def predict_video(video_path):
    ...
    while cap.isOpened():
        ...
        if frame_index % sample_rate == 0:
            # Resize frame to (224, 224) before processing
